...
- App Store submission gate: native checks now aggregate the commercial release blockers into a single Swift-verifiable gate covering full Xcode, Team ID, archive, TestFlight upload, support/privacy URLs, App Privacy questionnaire, Privacy Manifest required reason API review, 12+ age-rating review, signed-device Keychain/Photos/Files receipts, DeepSeek provider evidence, and deletion completion evidence
