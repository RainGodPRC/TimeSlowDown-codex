...
- Native build v88 replaces the latest-slice-only echo with a deterministic active-recall scheduler: due memories are selected from real sources by spaced-recall urgency, under-review protection, and source strength; the source stays hidden until the user actively recalls or asks for a cue; each day allows at most one completion or quiet skip, skips create a seven-day anti-nag cooldown, completed reviews can remain text-free, schema v5 persists the interaction ledger, and the default ZIP exports it without streak debt or scoring
- Native build v89 replaces the flat Slices list with a source-backed native Timeline: months become chapters, dates become rails, same-day moments stay newest-first, real photo/video anchors receive visual prominence without changing source order, and every card opens the existing editable memory detail; the local deterministic projection adds no vault schema migration and is covered at default and Accessibility XXL text sizes
- Native build v90 hardens that Timeline for decade-scale use: revisit totals are projected once per source slice instead of repeatedly filtering the complete revisit ledger while SwiftUI renders cards, source-backing validation now checks every per-slice projection, and a deterministic 3,650-day/7,300-revisit benchmark guards correctness and lookup cost without changing the vault schema
