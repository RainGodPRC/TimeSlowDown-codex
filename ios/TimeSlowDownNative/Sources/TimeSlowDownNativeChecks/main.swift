...
check(timelineJulySecond?.revisitCount(for: julyPhoto.id) == 1, "Timeline should project source-backed revisit counts onto each slice once")
check(timelineJulySecond?.revisitCount(for: julyOrdinary.id) == 0, "Timeline should expose zero revisits without rescanning the source collection")
...
check(projectText.contains("CURRENT_PROJECT_VERSION = 91;"), "Xcode project build settings should carry v91 build number")
...
check(infoPlistText.contains("<string>91</string>"), "Info.plist should carry v91 build number")
...
check(buildNotes.buildNumber == "91", "TestFlight build notes should match v91")
check(buildNotes.summary.localizedCaseInsensitiveContains("Beta Learning Release"), "TestFlight build notes should describe the v91 learning goal")
check(buildNotes.summary.localizedCaseInsensitiveContains("separate bounded local Beta Learning ledger"), "TestFlight build notes should disclose the privacy-separated learning ledger")
check(buildNotes.summary.localizedCaseInsensitiveContains("14-day experiment"), "TestFlight build notes should describe the Day 0/Day 14 experiment")
check(buildNotes.summary.localizedCaseInsensitiveContains("at most one prompt per day"), "TestFlight build notes should preserve restrained reminder frequency")
check(buildNotes.summary.localizedCaseInsensitiveContains("3,650-day, 7,300-revisit benchmark"), "TestFlight build notes should retain the v90 long-history benchmark")
check(buildNotes.summary.localizedCaseInsensitiveContains("Twelve XCUITests"), "TestFlight build notes should disclose all twelve simulator paths")
...
check(appStoreSubmissionGate.buildNumber == "91", "App Store submission gate should track v91")
check(appStoreSubmissionGate.rows.count == 30, "App Store submission gate should keep thirty release gates after v91 Beta learning instrumentation")
