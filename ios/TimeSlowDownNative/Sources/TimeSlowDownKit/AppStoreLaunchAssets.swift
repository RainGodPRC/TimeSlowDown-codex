...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        buildNumber: String = "47",
        summary: String = "TimeSlowDown v47 tests the native Memory Camera shell, media-first slice capture, weekly chapter preview, App Store launch assets, Keychain record store adapter, Account Rights export UI state, SwiftUI fileExporter bridge, on-device export ZIP builder, deletion audit envelope, DeepSeek server gateway envelope, deletion service boundary, and privacy/export/delete/AI trust boundaries.",
        testerRoute: [String] = [
