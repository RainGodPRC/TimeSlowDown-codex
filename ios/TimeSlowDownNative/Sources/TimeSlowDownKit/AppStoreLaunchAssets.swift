...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        knownLimitations: [String] = [
            ...
            "v81 defers thumbnail deletion until after a successful vault commit, but the app still lacks a durable multi-file journal for power-loss recovery during new-media creation; person/theme lenses, a persisted Life Marks provenance ledger, and signed-device accessibility receipts remain follow-up work.",
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v81 build notes name commit-after-save media garbage collection, versioned checksummed vault recovery, source-backed Meadow semantic zoom, protected image/video posters, export/delete rights, AI boundary, and production limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "81", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
