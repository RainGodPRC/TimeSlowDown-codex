...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        buildNumber: String = "55",
        summary: String = "TimeSlowDown v55 tests the native Memory Camera shell, media-first slice capture, Photos-library byte import adapter, E2EE media vault adapter, CryptoKit media vault envelope contract, Secure Enclave device-key contract, signed-device validation scaffold, weekly chapter preview, App Store launch assets, Keychain record store adapter, Account Rights export UI state, SwiftUI fileExporter bridge, on-device export ZIP builder, raw media export policy, staged raw media export builder, deletion audit envelope, DeepSeek server gateway envelope, DeepSeek provider validation scaffold, deletion service boundary, and privacy/export/delete/AI trust boundaries.",
        testerRoute: [String] = [
        ...
        knownLimitations: [String] = [
            "No production backend, account sync, or real DeepSeek provider call is bundled in this build; v55 separates pending backend, mock gateway, and future provider-passed validation receipts.",
