...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        knownLimitations: [String] = [
            ...
            "v82 separates domain and session persistence, but NativeShellStore remains a broad UI facade and the app still lacks a durable multi-file journal for power-loss recovery during new-media creation; person/theme lenses, a persisted Life Marks provenance ledger, and signed-device accessibility receipts remain follow-up work.",
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v82 build notes name domain/session state separation, schema v3 migration, revision-keyed persistence, post-commit media garbage collection, export/delete rights, AI boundary, and production limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "82", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
