...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    public static let currentBuildSummary = "TimeSlowDown v85 adds a formal XCUITest target and shared Xcode scheme with Debug-only deterministic fixtures. Three user-observable paths now run in the iOS Simulator: an honest empty vault can create a Quick Mark and find it in Slices; a seeded slice can be edited, deleted, and restored with undo; and Account memory/diagnostics export controls remain reachable at Accessibility XXL without terminating the app. The fixture store never reads or writes the production vault, diagnostics use an isolated temporary receipt store, and Release builds omit the fixture branch. "
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v85 build notes name the formal XCUITest target/shared scheme, Debug-only fixture isolation, three simulator paths, sanitized diagnostics boundary, and remaining signed-device/App Store limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "85", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
