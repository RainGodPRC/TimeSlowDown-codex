...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        buildNumber: String = "54",
        summary: String = "TimeSlowDown v54 tests the native Memory Camera shell, media-first slice capture, Photos-library byte import adapter, E2EE media vault adapter, CryptoKit media vault envelope contract, Secure Enclave device-key contract, signed-device validation scaffold, weekly chapter preview, App Store launch assets, Keychain record store adapter, Account Rights export UI state, SwiftUI fileExporter bridge, on-device export ZIP builder, raw media export policy, staged raw media export builder, deletion audit envelope, DeepSeek server gateway envelope, deletion service boundary, and privacy/export/delete/AI trust boundaries.",
        testerRoute: [String] = [
