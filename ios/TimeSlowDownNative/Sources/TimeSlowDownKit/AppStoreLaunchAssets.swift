...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        knownLimitations: [String] = [
            ...
            "v84 stores at most 200 sanitized operational receipts or 256KB locally and does not upload them; MetricKit delivery is aggregate-only and no XCUITest target or first-run onboarding is claimed yet. v83 keeps JSON documents encoded as bounded per-document Data and reads each thumbnail once for media-index metadata plus once for CRC/write streaming; it removes the all-thumbnails plus whole-archive memory peak, but it is not a zero-allocation exporter. NativeShellStore remains a broad UI facade and the app still lacks a durable multi-file journal for power-loss recovery during new-media creation; person/theme lenses, a persisted Life Marks provenance ledger, and signed-device accessibility receipts remain follow-up work.",
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v84 build notes name the sanitized bounded receipt ring, OSLog/MetricKit aggregation, user-initiated diagnostics export, URL-backed memory export, privacy boundaries, and remaining production limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "84", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
