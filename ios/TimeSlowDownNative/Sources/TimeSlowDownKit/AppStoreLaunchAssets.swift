...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    public static let currentTimelineSummary = "TimeSlowDown v91 is the Beta Learning Release: it tests whether people who do not enjoy journaling return to capture real moments and whether two weeks feel less blurred. A separate bounded local Beta Learning ledger stores only typed event names, timestamps, duration buckets, counts, and coarse states; it has no fields for memory text, people, photos, filenames, paths, or slice UUIDs and does not change memory-vault schema v5. The 14-day experiment separates Day 0 free recall, Day 14 free recall, and Timeline-assisted additional recall, saving only counts and 1–5 scales. Gentle local reminders require an explicit user action, use passive no-sound/no-badge delivery, remain fully optional, and schedule at most one prompt per day with no streak, debt, or penalty. v91 retains v90 decade-scale history projection and its deterministic 3,650-day, 7,300-revisit benchmark. Twelve XCUITests cover twelve user-observable paths, including the privacy-safe Day 0 flow, real-source navigation, and Accessibility XXL reachability. "
    public static let currentBuildSummary = "TimeSlowDown v88 adds deterministic active recall without turning memory into homework. The scheduler chooses only real, due slices and orders them by relative interval urgency, with fewer previous reviews, time unseen, and source strength as stable tie-breakers; the home card hides the original words until the user first tries to remember or explicitly asks for a cue. A review may be completed without writing, while ‘skip for now’ creates a seven-day local anti-nag cooldown rather than debt, loss, or a broken streak. Schema v5 persists a source-linked recall interaction ledger, migrates schema v4 in place with no synthetic history, removes and restores interactions with their source slice, and includes recall-interactions.json in both in-memory and URL-backed exports. Two new XCUITests bring the suite to nine user-observable paths and verify source concealment plus Accessibility XXL quiet-skip reachability. TimeSlowDown v87 turns Life Marks from temporary UI badges into a durable, source-backed provenance ledger. Every unlocked mark stores its unlock time plus the exact slice, media-anchor, and revisit IDs that caused it; tapping a mark opens the user's original words, image clue, and later reflection. Schema v4 persists the ledger, migrates schema v3 in place, rebinds or removes evidence when a source is deleted, restores it on undo, and includes life-marks.json in both in-memory and URL-backed exports. The gallery keeps mystery marks without totals, rankings, streak debt, or unexplained rewards. Two new XCUITests bring the suite to seven user-observable paths and prove a mark opens its real source detail at default and Accessibility XXL text sizes. TimeSlowDown v86 adds a progressive native first-run experience that creates one source-backed memory in about 60 seconds. The first screen keeps a single question and lets the user start with a photo/video, one person, or one small turning point; the text route needs only one sentence, the media route keeps a protected thumbnail, and success ends with the user's own first patch of meadow rather than another task. Account, subscription, age, notifications, full-library access, and AI are not prerequisites. Completion is stored in a separate versioned local marker so onboarding metadata never enters the memory vault, existing users are not intercepted, skips remain respected, and future marker schemas are never overwritten. TimeSlowDown v85 adds a formal XCUITest target and shared Xcode scheme with Debug-only deterministic fixtures. Five user-observable paths now run in the iOS Simulator, including the source-backed onboarding text route and Accessibility XXL onboarding choice reachability. The fixture store never reads or writes the production vault, diagnostics use an isolated temporary receipt store, and Release builds omit the fixture branch. "
    ...
    public init(
        buildNumber: String = "91",
        summary: String = "TimeSlowDown v84 adds a privacy-safe operational diagnostics loop for the shipping app. Vault load/recovery, versioned commits, post-commit media garbage collection, URL-backed memory export, system file export, and MetricKit delivery now produce bounded local receipts containing only typed outcomes, durations, counts, revisions, and coarse size buckets. Receipts never contain memory text, people, media bytes, UUIDs, filenames, paths, or raw Error descriptions; XCTest canaries lock that boundary. OSLog emits the same sanitized fields, MetricKit is summarized as aggregate payload/crash/hang/disk-write/CPU-exception counts without call stacks, and Account Rights lets the user explicitly export a separate JSON support receipt. The local ring is capped at 200 entries and 256KB, protected in Application Support, never uploaded, and excluded from the memory ZIP. TimeSlowDown v83 moves the shipping Account Rights export path from an in-memory ZIP to a URL-backed temporary file built off the main actor. Protected thumbnails are CRC-scanned and copied in bounded chunks, archive bytes stream directly to a private .partial file, and only a completed archive is atomically promoted to the system file exporter. The UI reports monotonic progress, supports cancellation, removes partial or completed temporary artifacts on failure, cancellation, the next serialized export, or file-exporter completion, and preflights available capacity immediately before the user-requested write. The Privacy Manifest declares NSPrivacyAccessedAPICategoryDiskSpace with Apple reason E174.1 solely to prevent an out-of-space export failure; capacity is not uploaded, persisted, profiled, displayed, or used for fingerprinting. The legacy in-memory ZIP builder remains compatibility-only and is no longer used by the shipping Account UI. TimeSlowDown v82 separates durable memory state from transient SwiftUI session state. Schema v3 stores only slices, revisits, and the privacy boundary in a checksummed NativeVaultPayload; the current tab, export summary, and temporary export errors reset on launch and never enter the vault. Existing schema v2 envelopes migrate in place with their original creation timestamp and memory data preserved while legacy session fields are intentionally discarded. Every successful domain mutation advances a small monotonic vault revision, while tab changes and export UI feedback do not; SwiftUI persistence now keys off that revision instead of comparing the entire store on every render. TimeSlowDown v81 closes the media deletion crash window: replacing, removing, or deleting a memory updates the snapshot first, commits the versioned vault, and only then garbage-collects managed thumbnails no longer referenced by that committed snapshot. Failed vault commits never delete media, shared thumbnails remain until their final committed reference disappears, and every commit returns an auditable garbage-collection receipt. A newly written thumbnail that never reaches a snapshot remains recoverable as an orphan and is cleaned by the next successful commit. TimeSlowDown v80 replaces the bare local store with a versioned native vault envelope carrying schema version, creation/update timestamps, and a canonical payload checksum. Every valid save rotates the prior vault into a last-known-good copy; checksum or file corruption restores that safe version while retaining the damaged primary for diagnosis. Legacy bare stores migrate in place, future schema versions are refused without moving or overwriting user data, and a formal SwiftPM XCTest target pins the migration and recovery contracts. TimeSlowDown v79 replaces the native Life Meadow placeholder with deterministic, source-backed semantic zoom across week, month, year, and decade. Quiet periods remain grass rather than being judged as failure; denser periods grow into flowers and trees only as a visual index of real slices, media anchors, and revisit layers. Every populated period opens the original words, protected image/video thumbnail, people, meaning, and every ‘now looking back’ reflection, while a plain chronological river preserves the same source path for users who prefer a linear view. The projection has stable calendar positions and IDs, supports Dynamic Type and VoiceOver descriptions, and never asks AI to invent a landscape. TimeSlowDown v78 hardens native persistence and media portability. A revisioned actor serializes debounced local persistence, flushes the latest snapshot when the app leaves the active scene, and prevents stale saves from overwriting newer memories. PhotosPicker photo and video imports use Transferable file representations; images are downsampled off the original file, AVFoundation performs protected video poster extraction, and the legacy raw-Data bridge is unavailable because it cannot prove metadata stripping. Default on-device export ZIP now includes available protected JPEG thumbnails plus indexed paths and checksums while continuing to exclude raw originals. The app does not load the entire video into Data or retain raw selected media in its memory vault. The build retains v77 protected video posters, v76 editable memory slices, media replacement/removal, missing-thumbnail recovery, undo deletion, first-launch empty vault, legacy migration, corrupt data backup recovery, the branded Memory Camera home, Yesterday Echo, native Weekly Story progress and Weekend Workbench, private Life Marks gallery, revisit export, Daily Difference Radar, 90-day tellable progress, Photos-library byte import adapter, E2EE media vault adapter, CryptoKit media vault envelope contract, Secure Enclave device-key contract, signed-device validation scaffold, signed-device media validation packet, archive/signing readiness packet, App Store metadata/legal review packet, screenshot/App Preview creative packet, Privacy Manifest required reason API audit packet, encryption export compliance review packet, Keychain record store adapter, Account Rights export UI state, SwiftUI fileExporter bridge, on-device export ZIP builder, raw media export policy, staged raw media export builder, deletion audit envelope, DeepSeek server gateway envelope, DeepSeek provider validation scaffold, DeepSeek integration test runner contract, DeepSeek backend endpoint/provider proxy contract, DeepSeek endpoint execution harness, optional live backend probe, deletion service boundary, deletion live probe, App Store submission gate, public URL packet, backend release manifest, App Privacy questionnaire packet, Age Rating review packet, and privacy/export/delete/AI trust boundaries.",
        testerRoute: [String] = [
            "Open My > 14-day memory validation, confirm the local-only privacy boundary, and save the Day 0 count-and-scale assessment without entering any memory content.",
            "Optionally enable Gentle Reminders and verify the app asks for notification permission only after the explicit button tap; disabling reminders creates no streak debt.",
            ...
            "Export the separate sanitized diagnostics receipt and confirm it contains no memory text, names, media, filenames, or paths.",
        ...
        knownLimitations: [String] = [
            ...
            "v84 keeps the Privacy Manifest required reason API audit packet current while operational receipts remain local-only and user-exported: OSLog and MetricKit summaries contain typed outcomes, duration/count metadata, and no memory content; PrivacyInfo remains no-tracking/no-off-device-collected-data while retaining the v83 DiskSpace E174.1 export preflight declaration. Any future automatic diagnostics upload requires a separate privacy and App Store Connect review.",
            ...
            "v91 adds the local-only Beta Learning ledger, 14-day memory experiment, and explicit passive reminder scheduler, but no completed real-user cohort, D1/D7/D14 benchmark, signed-device notification-delivery receipt, or production analytics upload is claimed. The ledger remains on device and a future off-device learning pipeline would require a separate consent, privacy, security, and App Store review. v90 adds a deterministic 3,650-day/7,300-revisit Timeline benchmark and removes the repeated full-ledger scan from card rendering, but search, map browsing, and device-class performance profiling remain follow-up work. v89 adds the deterministic month/day grouped native Timeline with source-backed media prominence. v88 adds deterministic active recall and an anti-nag interaction ledger. v87 persists source-backed Life Marks and exports their ledger, but the current catalog deliberately remains small and deterministic; richer user-authored firsts and person/theme lenses remain follow-up work. v86 automates the first-run text route and Accessibility XXL choice reachability, but Simulator automation still does not select real PhotosPicker media or produce signed-device Photos/Files receipts. No App Store/TestFlight upload is claimed. v84 stores at most 200 sanitized operational receipts or 256KB locally and does not upload them; MetricKit delivery is aggregate-only. v83 keeps JSON documents encoded as bounded per-document Data and reads each thumbnail once for media-index metadata plus once for CRC/write streaming; it removes the all-thumbnails plus whole-archive memory peak, but it is not a zero-allocation exporter. NativeShellStore remains a broad UI facade and the app still lacks a durable multi-file journal for power-loss recovery during new-media creation; signed-device accessibility receipts remain follow-up work.",
        ...
        self.summary = summary.hasPrefix("TimeSlowDown v84")
            ? Self.currentTimelineSummary + Self.currentBuildSummary + summary
            : summary
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v91 build notes name the local-only Beta Learning ledger, 14-day memory experiment, passive reminders, schema v5 memory portability, twelve simulator paths, and remaining real-cohort/signed-device/App Store limitations."),
...
public struct AppPrivacyQuestionnairePacket: Codable, Equatable, Sendable {
    ...
    public static let defaultAnswers: [AppPrivacyDataAnswer] = [
        ...
        .init(
            ...
            evidence: "v84 operational receipts are local-only unless the user explicitly exports them for support. They contain typed outcomes, durations, counts, and coarse buckets only—never raw memories, names, media, identifiers, filenames, paths, call stacks, or raw errors. Any future automatic upload requires separate disclosure and review."
        ),
        .init(
...
public struct PrivacyManifestRequiredReasonAPIAuditPacket: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        manifestHasNoAccessedAPITypes: Bool = false,
    ...
    public static let defaultItems: [PrivacyManifestRequiredReasonAPIAuditItem] = [
        ...
        .init(
            ...
            usedByShippingTarget: true,
            observedSymbols: ["URLResourceKey.volumeAvailableCapacityForImportantUsageKey in NativeMemoryExportFileBuilder"],
            approvedReasonIDs: ["E174.1"],
            evidence: "Used only immediately before a user-requested local ZIP export to prevent an out-of-space write failure.",
            notes: "Capacity is not uploaded, persisted, profiled, displayed, or used as device-fingerprinting data."
        ),
        .init(
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "91", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
