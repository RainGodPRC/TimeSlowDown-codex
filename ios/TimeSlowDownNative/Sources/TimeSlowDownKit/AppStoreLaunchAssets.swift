...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    public static let currentTimelineSummary = "TimeSlowDown v89 replaces the flat slice list with a month and day grouped native Timeline. Month chapters and day rails preserve every real slice in deterministic newest-first order, give source-backed photo or video anchors visual prominence without reordering the day, surface people and revisit context, and open the existing editable source detail. The projection is local, deterministic, lossless, and schema-neutral; quiet days remain visible without being scored. Two new XCUITests bring the suite to eleven user-observable paths and verify real-source navigation plus Accessibility XXL reachability. "
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v89 build notes name the grouped native Timeline, deterministic active recall, schema v5 portability, eleven simulator paths, and remaining signed-device/App Store limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "89", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
