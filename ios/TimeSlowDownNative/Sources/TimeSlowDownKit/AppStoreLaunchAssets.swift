...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    public static let currentTimelineSummary = "TimeSlowDown v90 makes the source-backed native Timeline safe for decade-scale history. The v89 month chapters, day rails, deterministic source order, media prominence, editable source detail, and quiet-day semantics remain unchanged, while each source slice now receives its revisit count once during Timeline projection instead of filtering the complete revisit ledger again for every rendered card. Source-backing validation checks every per-slice count, and a deterministic 3,650-day, 7,300-revisit benchmark guards lossless projection plus constant-time card lookup without changing the vault schema. The eleven XCUITests continue to cover eleven user-observable paths, including real-source navigation and Accessibility XXL reachability. "
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v90 build notes name the decade-scale Timeline projection, deterministic active recall, schema v5 portability, eleven simulator paths, and remaining signed-device/App Store limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "90", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
