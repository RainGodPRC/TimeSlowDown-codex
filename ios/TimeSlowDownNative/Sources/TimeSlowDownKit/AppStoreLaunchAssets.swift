...
public struct TestFlightBuildNotes: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        knownLimitations: [String] = [
            ...
            "v80 adds a versioned, checksummed vault and last-known-good recovery, but media files and JSON are not yet committed as one cross-file transaction; person/theme lenses, a persisted Life Marks provenance ledger, and signed-device accessibility receipts remain follow-up work.",
...
public enum AppStoreLaunchAssetChecklist {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "testflight-build-notes", title: "TestFlight build notes", status: .poc, owner: "release", evidence: "v80 build notes name versioned checksummed vault recovery, source-backed Meadow semantic zoom, the chronological river, revisioned persistence, protected image/video posters, portable thumbnail export, export/delete rights, AI boundary, and production limitations."),
...
public struct AppStoreSubmissionGate: Codable, Equatable, Sendable {
    ...
    public init(buildNumber: String = "80", rows: [AppStoreSubmissionGateRow]) {
        self.buildNumber = buildNumber
        self.rows = rows
    }
