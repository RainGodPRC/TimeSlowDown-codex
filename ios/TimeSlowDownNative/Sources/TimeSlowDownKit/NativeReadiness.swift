...
public enum NativeHandoffLedger {
    public static let rows: [ReadinessRow] = [
        ...
        .init(id: "deepseek-gateway", title: "DeepSeek gateway", status: .poc, owner: "backend", evidence: "v38 DeepSeek task envelope constrains provider, budget, fallback, and forbidden fields; v46 adds the server gateway envelope; v55 adds pending/mock/provider validation receipts so mock success cannot unlock production AI; v56 adds redacted integration test request/result contracts for future backend provider pass evidence; v57 adds the backend endpoint/provider proxy contract; real deployed gateway and provider pass receipt still required."),
